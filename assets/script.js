...
function changeDiffLevel(n) {
  switch(n)
  {
      case 1 :  window.difficulty = e[0];
      break;
      case 2 :  window.difficulty = m[0];
      break;
      case 3 :  window.difficulty = h[0];
      break;
      default: window.difficulty = e[0];
  }
  diffNum = n;
  start();
}
...
function high() {
  ...
  let col = parseInt(select.id % 9);
  let row = parseInt(select.id / 9);
  let s = select.id;
  for(let i=0;i<81;i++){
      if(parseInt(select.textContent)==answer[i])
...
function create(board) {
  ...
  for(let i=0;i<81;i++){
      ...
      if((cell.id > -1 && cell.id < 9)) {
          cell.classList.add("border_top");
      }
      id("game_board").appendChild(cell);
  }
}
function clear() {
  let cells = qA(".cell")
